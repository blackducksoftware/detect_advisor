...
'clang': "XXCLIOPTSXX    Note that Detect supports reading a compile_commands.json file generated by cmake.\n" + \
"XXCLIOPTSXX     If the project does not use cmake then it is possible to produce the compile_commands.json\n" + \
"XXCLIOPTSXX     from standard make using utilities such as https://github.com/rizsotto/Bear. Detect must be\n" + \
"XXCLIOPTSXX     run on Linux only for the detection of OSS packages using compile_commands.json, and the packages\n" + \
"XXCLIOPTSXX     must be installed in the OS.\n",
